...
const DIMENSION_COL = document.querySelectorAll(".grid")[0].scrollWidth    / document.querySelectorAll(".grid div")[0].scrollWidth
const DIMENSION_ROW = document.querySelectorAll(".grid")[0].scrollHeight   / document.querySelectorAll(".grid div")[0].scrollHeight
...
function checkIfSomeWinner(context){
    ...
    for (const key in Direction) {
        ...
        if(!checkOverflow(context.id, count, Direction[key]) && checkLimits(context.classList[0], context.id, count, Direction[key]))
            return true
    }
...
function checkOverflow(id, count, direction) {
    ...
    if(direction === Direction.Down){
        console.log("last id " + last_id + " limit to compare " + lower_limits[row_position])
...
function checkLimits(player_color, id, count, direction){
    ...
    return bResult === false ? false : document.getElementById(id).classList.contains(player_color)
}
